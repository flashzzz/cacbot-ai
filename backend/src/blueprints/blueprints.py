...
from src.routes.myProfile import get_user_profile, update_user_profile
from src.routes.routes import post_data_file, playground
from src.routes.auth import login, sign_up
...
myProfile_bp.add_url_rule(rule="/updateProfile/<name>", view_func=update_user_profile, methods=["POST"])

auth_bp.add_url_rule(rule="/auth/login", view_func=login, methods=["POST"])
auth_bp.add_url_rule(rule="/auth/signup", view_func=sign_up, methods=["POST"])
